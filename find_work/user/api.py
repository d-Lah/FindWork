...
class Register(APIView):
    def post(
        ...
        if email_exists:
            response_data = {"error": "EmailAlreadyExists"}
        ...
        if phone_number_exists:
            response_data = {"error": "PhoneNumberAlreadyExistsError"}
            return Response(response_data, status=409)

        if user_serializer.errors:
            return Response(
                {"error": f"UserFieldEmptyError"},
                status=400
            )
        ...
        if profile_serializer.errors:
            return Response(
                {"error": f"ProfileFieldEmptyError"},
                status=400
            )
        ...
        activate_user_url = f"http://localhost:8000/user/activate-user/"
